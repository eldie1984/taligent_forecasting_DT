...
class FeatureRowOld(BaseModel):
    county: str = Field(..., description="Name of the county")
    bottles: int = Field(..., ge=0, description="Number of bottles")
    ...
    is_quarter_start: int = Field(..., ge=0, le=1)
    is_quarter_end: int = Field(..., ge=0, le=1)
    y_lag_1: float
    y_lag_7: float
    y_lag_14: float
    y_lag_30: float
    y_rolling_mean_7: float
    y_rolling_std_7: float
    y_rolling_mean_14: float
    y_rolling_std_14: float
    y_rolling_mean_30: float
    y_rolling_std_30: float

class ForecastRequestOld(BaseModel):
    features: list[FeatureRowOld]

class ForecastRequest(BaseModel):
    target_date: str = Field(..., description="The date to forecast for, format YYYY-MM-DD")
    county: str = Field(..., description="The target county/country name matching BQ data")
...
def predict(payload: ForecastRequest):
    ...
    target_row_df = features_df[features_df['ds'].dt.strftime('%Y-%m-%d') == payload.target_date]
    ...
    try:
        ...
        job_config = bigquery.QueryJobConfig(
            query_parameters=[
                ...
                bigquery.ScalarQueryParameter("target_date", "STRING", payload.target_date),
            ]
        )
        
        logger.info(f"Executing BigQuery pull for county: {payload.county} around execution date: {payload.target_date}")
