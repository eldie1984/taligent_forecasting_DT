...
MLFLOW_EXPERIMENT_NAME = os.getenv(
    "MLFLOW_EXPERIMENT_NAME", "liquor_sales_forecasting"
)
...
def prepare_time_series_data(df):
    ...
    df_daily = df.groupby(['ordered_on', 'county_name']).agg({
        'sales_dollars': 'sum',
        'sales_bottles': 'sum',
        'sales_liters': 'sum',
        'invoice_id': 'count'
    ...
    for window in [7, 14, 30]:
        df_daily[f'y_rolling_mean_{window}'] = df_daily.groupby('county')['y'].transform(
...
def split_time_series_data(df, test_size=0.2):
    ...
    logger.info(f"Train set: {train_df.shape[0]} registros ({train_df['ds'].min()} a {train_df['ds'].max()})")
    logger.info(f"Validation set: {val_df.shape[0]} registros ({val_df['ds'].min()} a {val_df['ds'].max()})")
...
def train_xgboost_forecaster(train_df, val_df):
    ...
    feature_cols = [
        ...
        'y_lag_1', 'y_lag_7', 'y_lag_14', 'y_lag_30',
        'y_rolling_mean_7', 'y_rolling_std_7',
        'y_rolling_mean_14', 'y_rolling_std_14',
        'y_rolling_mean_30', 'y_rolling_std_30'
    ]
    ...
    logger.info(f"XGBoost forecaster entrenado")
...
def evaluate_forecast_model(val_df, forecasts, model_type="prophet"):
    ...
    logger.info(f"Métricas de forecasting:")
...
def generate_forecast_report(metrics, model_type, output_dir="results"):
    ...
    with open(report_path, "w") as f:
        ...
        if overfitting > 0.1:
            ...
            f.write(
                "Considerar técnicas de regularización o más datos históricos.\n"
            )
        else:
            f.write(
...
def setup_mlflow():
    ...
    logger.info("Configurando MLflow tracking...")
...
def log_parameters(params, random_state, test_size, n_iter, cv_folds, feature_list):
    """Registrar parámetros en MLflow"""
    logger.info("Registrando parámetros en MLflow...")

    # Hiperparámetros del modelo
    for param_name, param_value in params.items():
        param_clean = param_name.replace("model__", "")
        mlflow.log_param(f"model_{param_clean}", param_value)
    ...
    logger.info(f"Parámetros registrados: {len(params) + 5} parámetros")
...
def log_metrics(metrics_dict):
    ...
    for metric_name, metric_value in metrics_dict["train"].items():
        mlflow.log_metric(f"train_{metric_name}", metric_value)

    # Métricas de validación
    for metric_name, metric_value in metrics_dict["validation"].items():
        mlflow.log_metric(f"validation_{metric_name}", metric_value)

    # Overfitting score
    mlflow.log_metric("overfitting_score", metrics_dict["overfitting_score"])

    logger.info(
        f"Métricas registradas: {len(metrics_dict['train']) + len(metrics_dict['validation']) + 1} métricas"
    )


def log_artifacts(
    model, metrics, best_params, search_results, report_path, metadata_path
):
    """Registrar artefactos en MLflow"""
    logger.info("Registrando artefactos en MLflow...")

    # Registrar el modelo
    mlflow.sklearn.log_model(model, name="model")

    # Registrar el reporte de entrenamiento
    mlflow.log_artifact(report_path, "training_report.txt")

    # Registrar la metadata
    mlflow.log_artifact(metadata_path, "model_metadata.json")

    # Registrar resultados de búsqueda de hiperparámetros
    results_df = pd.DataFrame(search_results.cv_results_)
    results_path = "cv_results.csv"
    results_df.to_csv(results_path, index=False)
    mlflow.log_artifact(results_path, "cv_results.csv")
    os.remove(results_path)

    logger.info("Artefactos registrados: modelo, reporte, metadata, cv_results")


def save_best_model_artifacts(model, metrics, best_params, run_id, output_dir="models"):
...
def register_best_model(run_id, model, metrics):
    """Registrar el mejor modelo en MLflow Model Registry con etapa Staging"""
    logger.info("Registrando mejor modelo en MLflow Model Registry...")

    try:
        model_uri = f"runs:/{run_id}/model"
        model_name = MLFLOW_EXPERIMENT_NAME.replace("-", "_")

        # Registrar el modelo
        model_version = mlflow.register_model(
            model_uri=model_uri,
            name=model_name,
            tags={"framework": "xgboost", "task": "regression"},
        )

        # Transicionar el modelo a Staging
        client = mlflow.tracking.MlflowClient()
        client.transition_model_version_stage(
            name=model_name, version=model_version.version, stage="Staging"
        )

        # Agregar métricas como tags al modelo versionado
        client.set_model_version_tag(
            name=model_name,
            version=model_version.version,
            key="validation_rmse",
            value=str(metrics["validation"]["rmse"]),
        )
        client.set_model_version_tag(
            name=model_name,
            version=model_version.version,
            key="validation_r2",
            value=str(metrics["validation"]["r2"]),
        )

        logger.info(f"Modelo registrado en MLflow Model Registry como: {model_name}")
        logger.info(f"Versión {model_version.version} transicionada a etapa: Staging")
    except Exception as e:
        logger.warning(f"No se pudo registrar el modelo en Model Registry: {str(e)}")
...
def main():
    ...
    try:
        ...
        with mlflow.start_run() as run:
            ...
            # Paso 5: Entrenar modelo de forecasting
            if model_type == "prophet":
                ...
                # Log parameters específicos de Prophet
                mlflow.log_param("model_type", "prophet")
                mlflow.log_param("num_counties", len(prophet_models))
                mlflow.log_param("forecast_horizon", FORECAST_HORIZON)
                
            elif model_type == "xgboost":
                logger.info("Entrenando XGBoost forecaster...")
                xgb_model, y_train_pred, y_val_pred, feature_list = train_xgboost_forecaster(train_df, val_df)
                ...
                # Log parameters específicos de XGBoost
            ...
            # Paso 7: Registrar métricas en MLflow
            for metric_name, metric_value in metrics["train"].items():
                mlflow.log_metric(f"train_{metric_name}", metric_value)
            for metric_name, metric_value in metrics["validation"].items():
                mlflow.log_metric(f"validation_{metric_name}", metric_value)
            mlflow.log_metric("overfitting_score", metrics["overfitting_score"])

            # Paso 8: Registrar artefactos en MLflow
            mlflow.log_artifact(model_path, "forecast_model.pkl")
            mlflow.log_artifact(report_path, "forecast_report.txt")

            # Paso 9: Set tags para el run
            mlflow.set_tag("model_type", model_type)
            mlflow.set_tag("task", "time_series_forecasting")
            mlflow.set_tag("training_status", "completed")
            mlflow.set_tag("forecast_horizon", FORECAST_HORIZON)
